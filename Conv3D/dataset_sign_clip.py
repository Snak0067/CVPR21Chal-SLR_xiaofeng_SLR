...
class Sign_Isolated(Dataset):
    def __init__(self, data_path, label_path, frames=16, num_classes=226,
        ...
        for line in label_file.readlines():
            line = line.strip()
            line = line.split(',')
            complete_data_path = os.path.join(data_path, line[0])
            if not os.path.exists(complete_data_path):
                # logger.info(f"{complete_data_path} does not exist ")
                continue
    ...
    def read_images(self, folder_path, clip_no=0):
        ...
        for i in index_list:
            try:
                ...
                logger.info(f'image does not find {image_path}')
                continue
            if self.train:
                if flip_rand > 0.5:
